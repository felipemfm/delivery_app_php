...
if (!isset($_SESSION["userid"]) && (!isset($_POST["submit"]) || !isset($_POST["resubmit"]))) {
  ...
  if (isset($_POST["submit"])) {
    ...
    $_SESSION["store"]["open"] = $_POST["open"];
    $_SESSION["store"]["close"] = $_POST["close"];
  }
}
      ...
      <div class="inner inner--tPd0 flex shopping--flex">
        ...
        <article class="flex__main menu">
          <form action="./menu-cart.php" method="post">
            <div class="head head--flex head--sticky cart">
              <h2 class="head__title">メニュー</h2>
              <button class="head__link head__link--ponpon" type="submit" class="head-base"><i class="fas fa-shopping-cart"></i>カートを見る（<input id="total_item" name="個数" value='0' readonly>）</button>
              <div class="menu__total">
                <input id="total_price" name="合計金額" value='0' readonly><span>円 (税込)</span>
              </div>
            </div>
              ...
              <?php foreach ($menu as $element) : ?>
                <li class='list'>
                  ...
                  <div class='list__info'>
                    ...
                    <p class='list__price'><?php echo number_format($element['price']) ?>円<span class='list__price--small'>（税込）</span></p>
                  </div>
                  <div class='list__quantity'>
                    <span class="itemPrice" data-price='<?php echo $element['price'] ?>' style="display:none;"></span>
