...
$storeinfo = getStoreInfo($conn, $id, $_SESSION["lat"], $_SESSION["long"]);
      ...
      <div class="navigation__horizontal">
        <ul>
          ...
          <?php foreach ($category as $element) : ?>
            <?php echo ($id == $element['id']) ? "<li class='active'>" : "<li>" ?>
  ...
  <main class="main">
    ...
    <article class="wrap wrap--shopping">
      <div class="inner inner--tPd0">
        <h2 class="inner__title"><?php echo ($store_type) ?></h2>
        <section class="store">
          <?php foreach ($storeinfo as $element) : ?>
            <form action='menu.php' method='post' class='store__box'>
              <figure class='card'>
                <div class='card__img'>
                  <img src='../images/<?php echo $id ?>.jpg' loading='lazy'>
                </div>
                <div class='card__head'>
                  <figcaption class='card__title'><?php echo $element['name'] ?></figcaption>
                  <span class='card__icon'><?php echo getTimeToDelivery($_SESSION["lat"], $_SESSION["long"], $element["lat"], $element["long"]) ?>分</span>
                </div>
