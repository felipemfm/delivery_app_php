...
if (isset($_POST) && isset($_SESSION["userid"])) {
  ...
  $total = 0;
  $items = 0;
  foreach ($_POST as $order_string => $quantity) {
    if ($quantity != 0) {
      $order = explode("_", $order_string); //$order[0] => item code $order[1] => item name $order[2] => item price $order[3] => item quantity
      array_push($order, $quantity);
      array_push($_SESSION["cart"], $order);
      $total += ($order[2] * $quantity);
      $item += $order[3];
    }
  }
  ...
  <main class="main">
    <div class="wrap wrap--shopping">
      <div class="inner inner--tPd0 flex shopping--flexReverse">
          ...
          <section class="order-card">
            <?php if (count($_SESSION["cart"]) == 0) : ?>
              ...
              <ul class="order-card__total">
                ...
                <li class="order-card__total--price"><i class="fas fa-yen-sign"></i><?php echo number_format($total) ?><span>&nbsp;（税込）</span></li>
              ...
              <?php else : ?>
                <form action="../includes/shopping-order.inc.php" method="post">
                    ...
                    <dd>
                      <ul class="checkout__radio">
                        ...
                        <span id="radioSelect" class="checkout__radio--time">
                          <input type="time" name="time" value="<?php echo $date->format("H:i")?>" min="<?php echo $_SESSION["store"]["open"] ?>" max="<?php echo $_SESSION["store"]["close"] ?>">
                        </span>
                      </ul>
                    </dd>
                  </dl>
                  <ul class="order-card__total">
                    <li class="order-card__total--cart"><i class="fas fa-shopping-cart "></i><?php echo $item ?><span>&nbsp;品</span></li>
                    <li class="order-card__total--price"><i class="fas fa-yen-sign"></i><?php echo number_format($total) ?><span>（税込）</span></span></li>
        ...
        <!-- menu -->
        <article class="flex__main menu checkout">
          <?php if (count($_SESSION["cart"]) == 0) : ?>
          ...
          <?php else : ?>
                ...
                <div class="infoBg__box">
                  ...
                  <div class="flex">
                    ...
                    <ul class="infoBg__info">
                      ...
                      <li><i class="fas fa-clock"></i>注文可能時間:<span><?php echo $_SESSION["store"]["open"] . "-" . $_SESSION["store"]["close"] ?></span></li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <section class="checkout__item">
              ...
              <ul id="orderItem" class="flex menu--flex">
                <?php foreach ($_SESSION["cart"] as $order) : ?>
                  <li class="list">
                    ...
                    <div class="list__info">
                      ...
                      <p class="list__price"><?php echo number_format($order[2] * $order[3]) ?><span class="list__price--small">（税込）</span></p>
                    </div>
          ...
          <script>
            // list * 2 delete
            let parent = document.getElementById('orderItem');
            let children = document.getElementsByClassName('list');
            for (let i = 0; i < 2; i++) {
              ...
              <div class="cardBg">
                ...
                <div class="cardBg__box">
                  ...
                  <ul class="cardBg__info">
                    ...
                    <!-- <li class="cardBg__tel"><?php echo $_SESSION["store"]["open"]  ?></li> -->
                  </ul>
                </div>
              </div>
            </div>
