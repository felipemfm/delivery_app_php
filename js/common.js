...
$(function () {
  $("select").change(function () {
    ...
    for (var i = 0; i < $(".itemPrice").length; i++) {
      var item_price = $(".itemPrice").eq(i).data("price");
      var item_select = $(".itemPrice").eq(i).next("select").find("option:selected").data("num");
      totalArray.push(item_price * item_select);
      totalItemArray.push(item_select);
    }
    var total = 0;
    var totalItem = 0;
    for (var j = 0; j < totalArray.length; j++) {
      total += totalArray[j];
      totalItem += totalItemArray[j];
    }
