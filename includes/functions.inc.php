...
function invalidUsername($username){
    return !preg_match("/^[ぁ-んァ-ヶーa-zA-Z0-9一-龠０-９]+$/u", $username);
}
...
function creatUser($conn, $uid, $email, $pwd, $username, $postal, $address, $lat, $long)
{
    ...
    mysqli_stmt_bind_param($stmt, "ssssssss", $uid, $email, $hashedPwd, $username, $postal, $address, $lat, $long);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
function editUserProfile($conn, $edit, $userid, $type)
{
    if ($type == "user_password") {
        $edit = password_hash($edit, PASSWORD_DEFAULT);
    }
...
function getStoreInfo($conn, $id, $lat, $long)
{
    $storeinfo = array();
    $param = "%{$id}%";
    $sql = "SELECT * FROM business_database WHERE category_id LIKE ?;";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) header("location: ../shopping?error=stmtFailed");
    ...
    if ($result = $stmt->get_result()) {
        while ($row =  $result->fetch_assoc()) {
            ...
            $entry['open'] = $row['store_open'];
            $entry['close'] = $row['store_close'];
...
{
    $menu = array();
    $sql = "SELECT * FROM menu_database WHERE category_id = ? GROUP BY item_code ORDER BY item_code ASC";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) header("location: ../shopping?error=stmtFailed");
